...
<br>
    ...
    **`Path环境变量`**配置方法如下：
    ...
    然后按**`Ctrl+X`**退出nano，由于对文件做了修改，退出前nano会询问是否保存修改，此时再按**`Y`**确认保存并退出，回到终端界面中。之后关闭这个终端，再重启新的终端，重输入**`which hugo`**命令后，应该就会返回Hugo的保存路径，即Hugo可以开始工作了。
...
最省心的网站配置方式是直接用 **`kiera`**文件夹下、**`exampleSite`**文件夹中的 **`config`**文件以及 **`content`**文件夹替换 **`MyBlog`**内的 **`config`**文件和 **`content`**文件夹，后面再根据自己的实际情况修改替换后的 **`config`**、以及 **`content`**下面的文件。这里只做替换，先快速看看网站的雏形。
...
我们可以在**Visual Studio Code**中创建**Markdown**文件并保存在 **`posts`**内，也可以用下面的Hugo命令直接新建**Markdown**文件。
